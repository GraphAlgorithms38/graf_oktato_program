// Magyar módszer algoritmus lépéseinek kezelése páros gráfban (augmentáló utak, Kőnig-tétel).
// Bemenet: A (N×N) mátrix, A[i][j] > 0 <-> él van L_i (alsó) és U_j (felső) között.
// Kimenet: steps[] – { phase, msg, tables?, graph? }
...
export function hungarianSteps(A) {
  ...
  // Párosítás: matchU[j] = i, matchL[i] = j ( -1 = szabad )
  const matchL = Array(n).fill(-1);
  const matchU = Array(n).fill(-1);

  const steps = [];

  // segéd: lépés hozzáadása táblázatokkal és vizualizációs infóval
  ...
  function graphState({ labL = null, labU = null, frontier = null } = {}) {
    // Alap: minden él (i,j) ahol adj[i][j] igaz
    const edges = [];
    for (let i = 0; i < n; i++)
    ...
    return {
      ...
      n,
      edges, // összes él
      matched, // párosított élek
      labelsL, // L oldali címkézettek
      labelsU, // U oldali címkézettek
      frontierL, // sorban lévő L csúcsok
      frontierU, // sorban lévő U csúcsok
    };
  }
  ...
  // Egy augmentáló iteráció: címkézés és keresés úgy mint a BFS-ben
  function findAndAugment() {
    ...
    // Nincs javító út -> maximális párosítás. Minimális lefogó ponthalmaz
    const coverL = Array(n).fill(false);
    const coverU = Array(n).fill(false);
    for (let i = 0; i < n; i++) coverL[i] = labL[i] < 0; // alsó címkézetlen
    for (let j = 0; j < n; j++) coverU[j] = labU[j] >= 0; // felső címkézett
